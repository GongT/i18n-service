...
export const path = new JsonApiHandler<PathAdd, ApiResponse>(ERequestType.TYPE_PUT, '/' + PathApi);
path.handleArgument('path').fromGet().filter((d) => {
	return d.length > 0 && !/^\.|\.$/.test(d);
});
path.setHandler(async (context) => {
	const data = await getDatabase().writeKeyAll(context.params.path, context.params.path);
	await refreshLanguageList();
	return {};
});

export const path_del = new JsonApiHandler<PathAdd, ApiResponse>(ERequestType.TYPE_DELETE, '/' + PathApi);
path_del.handleArgument('path').fromGet().filter((d) => {
	return d.length > 0 && !/^\.|\.$/.test(d);
});
path_del.setHandler(async (context) => {
	const data = await getDatabase().writeKeyAll(context.params.path, undefined);
	await refreshLanguageList();
	return {};
});
